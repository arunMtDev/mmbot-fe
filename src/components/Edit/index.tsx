...
const Edit = () => {
  ...
  const editTradeSchedule = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    if (lotSizeError !== "" || limitError !== "" || intervalError !== "") {
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      toast.success("Trade Schedule is edited successfully");
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setLotSize(data.tradeQuantity);
        setTradeSide(data.side);
        setLimit(data.numberOfTrades);

        const { hours, minutes, seconds } = convertIntervalTime(data.interval);
        setIntervalHours(hours);
        setIntervalMinutes(minutes);
        setIntervalSeconds(seconds);

        const exchangesObject = convertExchangesToObject(data.exchanges);
        setCheckbox1(!!exchangesObject["Azbit"]);
        setCheckbox2(!!exchangesObject["Coinsbit"]);
        setCheckbox3(!!exchangesObject["DexTrade"]);
        setStartTime(new Date(data.startTime));
      } catch (error) {
        console.error("Error fetching trade schedule:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    console.log("vsycfgasjycgsjyvgh");
    if (checkbox3 && lotSize < 10) {
      setLotSizeError(
        "Lot size must be 10 or greater when Dex-Trade is selected."
      );
    } else if (lotSize <= 0) {
      setLotSizeError("Lot size must be greater than 0");
    } else {
      setLotSizeError("");
    }
  }, [checkbox3, lotSize]);

  const convertExchangesToObject = (
    selectedExchanges: string[]
  ): { [key: string]: boolean } => {
    const exchanges: { [key: string]: boolean } = {};
  ...
  useEffect(() => {
    const interval: number = calculateIntervalTime();
    if (interval < 1000) {
      setIntervalError("Interval must be greater than or equal to 1 second");
    } else {
      setIntervalError("");
    }
  }, [intervalHours, intervalMinutes, intervalSeconds]);

  const convertIntervalTime = (
    interval: number
  ...
  return (
    <section className="py-[100px]">
      <div className="container mx-auto  max-[1079px]:px-4">
        <p className="text-2xl text-[#000] font-bold">Edit Trade Schedules</p>
        ...
        <div className="">
          ...
          {limitError && <p className="text-red-600 my-2">{limitError}</p>}
          <div className="mb-4 flex  flex-col">
            <div className="lg:items-center lg:flex-row flex-col flex-row flex lg:gap-3 gap-1">
              <label className="block md:text-lg sm:text-base text-sm text-[#000] font-normal">
                Start After:
          ...
          {intervalError && (
            <p className="text-red-600 my-2">{intervalError}</p>
          )}
